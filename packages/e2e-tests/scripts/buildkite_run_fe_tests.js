...
function run_fe_tests(CHROME_BINARY_PATH, runInCI = true) {
  ...
  if (!CHROME_BINARY_PATH) {
    console.warn("No chromium binary path (nor REPLAY_CHROMIUM_EXECUTABLE_PATH) provided. Using system default.");
  }
  if (CHROME_BINARY_PATH) {
  ...
  // Start the webserver.
  const webProc = exec("yarn dev", (error, stdout, stderr) => {
    if (error) {
  ...
  console.time("SAVE-EXAMPLES time");
  {
    ...
    execSync(
      `${envWrapper} ${path.join(TestRootPath, "scripts/save-examples.ts")} --runtime=chromium --target=browser --project=chromium --example=${htmlFiles.join(",")}`,
  ...
  console.time("TESTS time");
  {
    ...
    execSync(`${envWrapper} playwright test --grep-invert node_ --project=chromium --workers=1 --retries=2 ${testNames.join(" ")}`, {
      stdio: "inherit",
      env: process.env,
    });
