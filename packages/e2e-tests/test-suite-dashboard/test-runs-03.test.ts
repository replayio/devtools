...
test(`authenticated/new-test-suites/test-runs-03: flaky run in main branch with source`, async ({
  ...
  // >>> A test run in which all tests are flaky should display a green checkmark to the left of the test run title
  ...
  // >>> trigger author, branch name, workflow link pull request number and link should be displayed
  ...
  // >>> The icon should be the merged branch icon
  expect(await testRunBranch.getAttribute("data-test-branch")).toBe("primary");

  // > Replay List

  // >>> Replay list should be displayed with both passed and flaky recording
