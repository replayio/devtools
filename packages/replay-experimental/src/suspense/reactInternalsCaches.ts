...
// TODO These hacks can go away once we have ReactDOM sourcemaps,
// as then we can just look for these functions by name in source outlines
...
> = createCache({
  ...
  load: async ([replayClient, functionName]) => {
    ...
    if (!functionDetails) {
      ...
      // TODO This can all go away entirely once React has sourcemaps
...
export const reactInternalMethodsHitsIntervalCache = createFocusIntervalCacheForExecutionPoints<
  ...
  PointDescription
>({
  debugLabel: "ReactInternalMethodHits",
