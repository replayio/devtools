...
test("should display toggleable stack for warnings", async ({ page }, testInfo) => {
  await setup(page);
  await toggleProtocolMessage(page, "warnings", true);

  const listItem = await locateMessage(page, "console-warning", "This is a warning");
  await takeScreenshot(page, testInfo, listItem, "warning-stack-collapsed");

  const toggle = listItem.locator("[role=button]", { hasText: "This is a warning" });
  await toggle.click();
  await takeScreenshot(page, testInfo, listItem, "warning-stack-expanded");
});
