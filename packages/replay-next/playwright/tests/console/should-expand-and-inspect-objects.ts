...
test("should expand and inspect objects", async ({ page }, testInfo) => {
  await setup(page);
  await toggleProtocolMessage(page, "errors", true);

  const listItem = await locateMessage(page, "console-error", "This is an error");
  await takeScreenshot(page, testInfo, listItem, "object-collapsed");
  ...
  await takeScreenshot(page, testInfo, listItem, "object-expanded");
  ...
  await takeScreenshot(page, testInfo, listItem, "nested-object-expanded");
});
