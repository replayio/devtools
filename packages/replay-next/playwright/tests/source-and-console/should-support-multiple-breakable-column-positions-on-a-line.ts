...
test("should support multiple breakable column positions on a line", async ({ page }, testInfo) => {
  const lineNumber = 4;
  const lineLocator = getSourceLineLocator(page, sourceId, lineNumber);

  await takeScreenshot(page, testInfo, lineLocator, "line-4-no-breakpoints-enabled");

  await addBreakPoint(page, { lineNumber, sourceId });
  await stopHovering(page);
  await takeScreenshot(page, testInfo, lineLocator, "line-4-break-on-column-19");

  await toggleColumnBreakpoint(page, true, { columnIndex: 22, lineNumber, sourceId });
  await stopHovering(page);
  await takeScreenshot(page, testInfo, lineLocator, "line-4-break-on-columns-19-and-22");
  ...
  await stopHovering(page);
  await takeScreenshot(page, testInfo, lineLocator, "line-4-break-on-columns-19-and-30");
