...
>(
  ...
  async (client, begin, end, location, code, condition) => {
    const locations = client.getCorrespondingLocations(location);
    ...
    await Promise.all(
      locations.map(async location => {
        ...
        points = points.concat(pointDescriptions.map(({ point, time }) => ({ point, time })));
      })
    );
    return points;
  },
  (points, location, code, condition) => ({
    selector: {
      kind: "points",
...
export function getLogPointAnalysisResultSuspense(
  ...
  if (canRunLocalAnalysis(code, condition)) {
    ...
    const mappedCode = mappedExpressionCache.read(client, `[${code}]`, location);
    const mappedCondition = condition
      ? mappedExpressionCache.read(client, condition, location)
      : null;
