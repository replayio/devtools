...
export function rangesOverlap(range1: Range, range2: Range) {
  return range1.end > range2.begin && range1.begin < range2.end;
}

export function rangeContains(containerRange: Range, range: Range) {
  return containerRange.begin <= range.begin && containerRange.end >= range.end;
}
