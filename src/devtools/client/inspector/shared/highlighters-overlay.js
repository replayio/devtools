...
class HighlightersOverlay {
  ...
  canGridHighlighterToggle(node) {
    const maxGridHighlighters = Services.prefs.getIntPref("devtools.gridinspector.maxHighlighters");
    return (
  ...
  onShapesHighlighterHidden(data) {
    this.emit("shapes-highlighter-hidden", this.shapesHighlighterShown, this.state.shapes.options);
    this.shapesHighlighterShown = null;
    this.state.shapes = {};
  }
  ...
  async getFlexboxHighlighterColor() {
    ...
    let hostname;
    try {
      hostname = parseURL(this.target.url).hostname || parseURL(this.target.url).protocol;
    } catch (e) {
  ...
  async hideGridHighlighter(node) {
    ...
    for (const highlightedNode of this.gridHighlighters.keys()) {
      const parentGridNode = await this.walker.getParentGridNode(highlightedNode);
  ...
  async hideGeometryEditor() {
    if (!this.geometryEditorHighlighterShown || !this.highlighters.GeometryEditorHighlighter) {
      return;
    }
  ...
  async _getGridHighlighter(node, isParent) {
    ...
    if (this.extraGridHighlighterPool.length > 0) {
      ...
      try {
        highlighter = await this.inspectorFront.getHighlighterByType("CssGridHighlighter");
      } catch (e) {
        this._handleRejection(e);
      }
    }
  ...
  isRuleViewShapePoint(nodeInfo) {
    ...
    const isShape =
      ...
      (nodeInfo.value.property === "clip-path" || nodeInfo.value.property === "shape-outside");
    const isEnabled =
      nodeInfo.value.enabled && !nodeInfo.value.overridden && !nodeInfo.value.pseudoElement;
    return (
      isShape &&
  ...
  async onDisplayChange(nodes) {
    for (const node of nodes) {
      ...
      // container.
      if (display !== "flex" && display !== "inline-flex" && node == this.flexboxHighlighterShown) {
        await this.hideFlexboxHighlighter(node);
        return;
      }
      ...
      if (
        display !== "grid" &&
        display !== "inline-grid" &&
        (this.gridHighlighters.has(node) || this.parentGridHighlighters.has(node))
      ) {
        await this.hideGridHighlighter(node);
        return;
      }
  ...
  async onMarkupMutation(mutations) {
    ...
    await this._hideHighlighterIfDeadNode(this.shapesHighlighterShown, this.hideShapesHighlighter);
  }
