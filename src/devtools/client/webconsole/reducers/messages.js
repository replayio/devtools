...
function maybeSortVisibleMessages(state, sortWarningGroupMessage = false, timeStampSort = false) {
  ...
  if (state.hasExecutionPoints) {
    state.visibleMessages = [...state.visibleMessages].sort((a, b) => {
      const _a = JSBI.BigInt(messageExecutionPoint(state, a));
      const _b = JSBI.BigInt(messageExecutionPoint(state, b));
      if (JSBI.lessThan(_a, _b)) {
        return -1;
      } else if (JSBI.greaterThan(_a, _b)) {
        return 1;
      } else {
