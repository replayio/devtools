  ...
  useLayoutEffect(() => {
    async function fetchAction() {
      switch (selectedTab) {
        case "action":
        case "state": {
          const res = await reduxActionStateCache.readAsync(point, time, replayClient);
          setReduxValues(res ?? null);
          break;
        }
        case "diff": {
          const diffRes = await reduxDiffCache.readAsync(point, time, replayClient);
