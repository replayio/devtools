...
export function PlanDetails({ subscription }: { subscription: SubscriptionWithPricing }) {
  return (
    <>
      ...
      <div className="flex flex-row items-center justify-between border-b border-themeBorder py-2">
      ...
      {subscription.billingSchedule && subscription.billingSchedule !== "contract" && (
        <>
          ...
          {subscription.discount ? (
            <>
              <div className="flex flex-row items-center justify-between border-b border-themeBorder py-2 font-bold">
                <span>Subtotal</span>
                <span>{cycleSubtotal(subscription)}</span>
              </div>
              <div className="ml-3 flex flex-row items-center justify-between border-b border-themeBorder py-2">
