...
export default class KeyShortcuts {
  ...
  private attachedTarget: EventTarget | undefined;

  constructor(listeners?: Record<string, KeyboardEventListener>) {
    if (listeners) {
  ...
  attach(target: EventTarget) {
    ...
    this.attachedTarget.addEventListener("keydown", this);
  }

  detach() {
    if (this.attachedTarget) {
      this.attachedTarget.removeEventListener("keydown", this);
  ...
  on(key: string, listener: KeyboardEventListener) {
  ...
  off(key: string, listener: KeyboardEventListener) {
  ...
  private static parseElectronKey(str: string) {
    ...
    const shortcut: Partial<Shortcut> = {
      ctrl: false,
      meta: false,
      alt: false,
      shift: false,
  ...
  private doesEventMatchShortcut(event: KeyboardEvent, shortcut: Shortcut) {
    ...
    return (
      ...
      (shortcut.key && shortcut.key.match(/[0-9]/) && event.keyCode == shortcut.key.charCodeAt(0))
    );
  }
}
